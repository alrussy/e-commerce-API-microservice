...
import com.alrussy_dev.inventory_service.model.dto.Event.CancelledInvoiceProcurementEvent;
...
@Slf4j
public class CancelledProcurementEventLisener {
    ...
    private void orderCreated(CancelledInvoiceProcurementEvent event) {
        log.info(
                " Event listener... successful orderNumber : {} ,Line Products : {}",
                event.number(),
                event.lineProducts());
        inventoryService.action(
                new ActionRequest(event.number(), ActionType.REMOVE_STOCK, "Return Procurement", event.lineProducts()));
    }
}
