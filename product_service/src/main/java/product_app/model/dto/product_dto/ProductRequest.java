...
public record ProductRequest(@NotBlank(message = "name is blank it must be content one letter  ") String name,Boolean isFeature,
		@NotNull Long categoryId,@NotNull Long departmentId,@NotNull Long brandId,@NotBlank String imageUrl,String about) {
	...
	public Product mapToProduct() {
		...
		return Product.builder().id(id).name(name)
				.isFeature(isFeature==null?false:isFeature).brand(Brand.builder().id(brandId).build()).department(department).brandCategory(brandCategory)
				.imageUrl(imageUrl).about(about).build();
	}
