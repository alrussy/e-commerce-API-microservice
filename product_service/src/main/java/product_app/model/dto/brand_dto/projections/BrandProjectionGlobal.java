...
default BrandResponse mapToBrndResponse() {
	return new BrandResponse(getId(), getName(), getImage_Url(), getIs_Feature(),getProduct_count(),
			getCategory_names()!=null?getCategory_names().stream().map(t->t.mapToCategoryResponse()).toList():null);
}
